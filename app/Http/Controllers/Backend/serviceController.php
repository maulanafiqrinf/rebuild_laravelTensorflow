...
class serviceController extends Controller
{
    ...
    {
        try {
            // Mengambil data client dengan pagination dan pencarian
            $query = client::query();
            $services = $query->where('category', 'service')->get();

            // Mengoper data ke view
            $title = 'Layanan';
            return view('admin.service.index', compact('services', 'title'));
        } catch (Exception $e) {
            return redirect()->route('services.index')->with('error', 'Terjadi kesalahan saat memuat data.');
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            return redirect()->route('services.index')->with('error', 'Terjadi kesalahan saat menambahkan data.');
        }
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            return redirect()->route('services.index')->with('error', 'Terjadi kesalahan saat memperbarui data.');
        }
    }
